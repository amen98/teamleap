		...
		<div class="container">
			<div class="flex-w flex-tr">
				<div class="size-210 bor10 p-lr-70 p-t-55 p-b-70 p-lr-15-lg w-full-md">
					...
					<form>
					<h4>Les détails de votre livraison </h4><br><br>
...
$listeLivraisons=$livraison1C->recupererLivraison1($_GET['idCommande']);
...
foreach($listeLivraisons as $row){
  ...
  $transporteurC=new TransporteurC();
    $result=$transporteurC->recupererTransporteur2($row['idTransp']);
...
$your_date = strtotime($row['dateL']);
$datediff =  $your_date - $now;
$nbJ=round($datediff / (60 * 60 * 24));


?>
	<p>Livraison N°<?PHP echo $row['num']; ?></p>
	<p>Commande N°<?PHP echo $row['idCommande']; ?></p>
	<?php
if ($nbJ >= 1){
	?>
	<p><?PHP echo $row['dateL'];?><font size="2"> ( livraison dans <?php echo $nbJ ?> jour(s) )</p>
<?php
}
else if ($nbJ == 0){
	?>
	<p><?PHP echo $row['dateL'];?><font size="2"> ( Livraison dans 24h )</p>
	<?php
}
else{
	?>
	<p><?PHP echo $row['dateL'];?><font size="2"> ( Votre commande a été livré )</p>
	<?php
}
?>
	<p><?PHP echo $row['adresse'].'-'.$row['Destination']; ?></p></br>
<h5>Détails:</h5>
<p>CIN Client:<?PHP echo $row['cinClient']; ?></p>
<p>Transporteur:<a href="AfficherTransporteur.php?idTransp=<?PHP echo $row['idTransp'];?>&idCommande=<?PHP echo $row['idCommande']?>"><?PHP echo $nom.' '.$prenom; ?></abbr></a><font size="2"> (Cliquer pour plus d'info)</font></p>
<p>Frais de livraison:<?PHP echo $row['frais'].' Dinars Tunisien' ?></p>
</br>
<?php
if ($nbJ >= 3){
	?>

<table border="0px" id="dataTable2" width="100%" cellspacing="0">
	<tr>
		<td>
			
		<a href="ModifierLivraisonDetails.php?num=<?PHP echo $row['num']; ?>">Modifier la livraison</a></td>
<td><a href="supprimerLivraison.php?num=<?PHP echo $row['num']; ?>">Annuler la livraison</a>
</td>
...
<?php
	...
	<form action="#" method="POST">
													...
													<a href="NoterTransp.php?note=1&idCommande=<?PHP echo $row['idCommande']?>&idTransporteur=<?PHP echo $row['idTransp']?>"><i class="item-rating pointer zmdi zmdi-star-outline"></i>
													<a href="NoterTransp.php?note=2&idCommande=<?PHP echo $row['idCommande']?>&idTransporteur=<?PHP echo $row['idTransp']?>"><i class="item-rating pointer zmdi zmdi-star-outline"></i>
													<a href="NoterTransp.php?note=3&idCommande=<?PHP echo $row['idCommande']?>&idTransporteur=<?PHP echo $row['idTransp']?>"><i class="item-rating pointer zmdi zmdi-star-outline"></i>
													<a href="NoterTransp.php?note=4&idCommande=<?PHP echo $row['idCommande']?>&idTransporteur=<?PHP echo $row['idTransp']?>"><i class="item-rating pointer zmdi zmdi-star-outline"></i>
													<a href="NoterTransp.php?note=5&idCommande=<?PHP echo $row['idCommande']?>&idTransporteur=<?PHP echo $row['idTransp']?>"><i class="item-rating pointer zmdi zmdi-star-outline"></i>
	...
	<p><font color="red" size="1px">*Vous ne pouvez pas modifier ou annuler la livraison parce qu'il reste moins de 3 jours a la livraison.</font></p>
<?php
}
?>
	<?PHP
...
if ($i == 0 ){
	?>
	<a href="ajouterLivraison.php?idCommande=<?php echo $_GET['idCommande'];?>">Effectuer une demmande de livraison a domicile</a>
	<?php
}
?>
...
<tr>
 ...
 <p id="demo"></p>
	...
	<br><p><font size="2"><b>*NB:</b></font><font size="1"> Chaque modification ou annulation d'une livraison doit être avant 3 jours</font></p>
